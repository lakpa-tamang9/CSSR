...
def find_anchor_means(net, mapping, datasetName, trial_num, cfg, only_correct=False):
    ...
    if datasetName == 'MNIST' or datasetName == "SVHN":
        ...
        logits, labels = gather_outputs(
            net, mapping, loader, only_correct=only_correct)
    else:
        loader, loaderFlipped = dataHelper.get_anchor_loaders(
            datasetName, trial_num, cfg)
        logits, labels = gather_outputs(
            net, mapping, loader, loaderFlipped, only_correct=only_correct)
    ...
    if dataloaderFlip is not None:
        for i, data in enumerate(dataloaderFlip):
            ...
            if unknown:
                ...
                targets = torch.Tensor([mapping[x]
                                       for x in labels]).long().cuda()
